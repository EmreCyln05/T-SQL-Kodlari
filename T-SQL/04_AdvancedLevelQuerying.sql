...
--	ic ice Alt Sorgular Olusturmak

--	Tekil Degerler Donduren ic ice Sorgular
...
--	Coklu Sonuc Donduren ic ice Sorgular
...
--	SELECT Listesindeki İliskili Alt Sorgular
...
--	WHERE Kosulundaki İliskili Alt Sorgular
...
--	EXISTS(Alt sorgu en az 1 satır döndürüyorsa → TRUE) ve 
--  NOT EXISTS(Alt sorgu hiç satır döndürmüyorsa → TRUE)
...
--	Veri Tiplerini Donuşturmek: CAST ve CONVERT
...
--	ROW_NUMBER(Sonuç kümesindeki her satıra farklı bir numara verir.
--             Aynı değerler olsa bile numara tekrar etmez.)
...
--	RANK(Veriyi ORDER BY ile sıralar,
--       Aynı değerlere aynı rank verir,
--       Eşitlik varsa sonraki sıra numarası atlanır.)
...
--	DENSE_RANK(Veriyi sıralar (ORDER BY), 
--             Aynı değerlere aynı sıra numarasını verir, 
--			   Sonraki sırayı atlamadan devam ettirir.)
...
--	NTILE(Sonuçları N adet gruba ayırır.)
...
--	TABLESAMPLE(Tablonun tamamını değil, bir kısmını okur. Rastgele veri döndürür.)
...
--	PIVOT(Bir kolondaki kategorileri, Ayri ayri sutunlar haline getirir)
...
--	UNPIVOT(Birden fazla kolonu, Tek bir kolon + değer yapısına dönüştürür)
...
--	INTERSECT(iki sorgunun ortak olan kayitlarini getirir.)
...
--	EXCEPT(ilk sorguda olup ikinci sorguda olmayan kayitlari getirir.)
...
--	Tabloları Birlestirerek Veri Guncellemek
...
--	Alt Sorgular İle Veri Guncellemek
...
--	Buyuk Boyutlu Verileri Guncellemek
...
VALUES(1,'www.xbank.com','Online eğitim');
...
--	Tablo Birlestirerek Veri Silmek
...
VALUES(1,'Emre','CEYLAN',1),(2,'Eray','ERAYDIN',2),(3,'Can','CANSEVER',2);
...
--	Silinen Bir KaydIn DELETED içerisinde Goruntulenmesi
...
--	Dosyaların Veritabanına Eklenmesi ve Guncellenmesi
...
INSERT UserFiles(UserID, UserFile)
SELECT 1, BulkColumn 
FROM OPENROWSET(BULK'C:\XBank_logo.png',SINGLE_BLOB) AS UserImageFile;
...
--	FILESTREAM Ozelliğini Aktifleştirmek
...
--	Mevcut Bir Veritabanında FILESTREAM Kullanmak
...
INSERT INTO ADVDocuments(DocGUID, DocFile, DocDesc)
		VALUES(NEWID(), 
		(SELECT * 
		 FROM OPENROWSET(BULK N'C:\XBank.docx', SINGLE_BLOB) AS Docs), 		 'XBank Tanıtım Dökümanı');



--	FILESTREAM Ozelliği Aktif Edilmiş Bir Veritabanı Oluşturmak



CREATE DATABASE XBankLabs
ON
PRIMARY(
	NAME = XBankLabsDB,
	FILENAME = 'C:\Databases\XBankLabs\XBankLabsDB.mdf'
),
FILEGROUP XBankLabsFS CONTAINS FILESTREAM(
	NAME = XBankLabsFS,
	FILENAME = 'C:\Databases\XBankLabs\XBankLabsFS'
)
LOG ON(
	NAME = XBankLabsLOG,
	FILENAME = 'C:\Databases\XBankLabs\XBankLabsLOG.ldf'
);


--	Sutunları Oluşturmak
...
SELECT NEWID(),
	 'Emre',
	 'CEYLAN',
	 'emre.ceylan@mail.com',
	 CAST(BulkColumn AS VARBINARY(MAX))
	 FROM OPENROWSET(BULK 'C:\XBank.png',SINGLE_BLOB) AS ImageData;
...
--	GUID Degere Sahip Sutun Olusturulurken Dikkat Edilmesi Gerekenler
...
--	Verileri Gruplamak ve Ozetlemek
...
--	Having İle Gruplamalar Ustunde Sart Kosmak
...
--	GROUPING ile Ozetleri Duzenlemek
